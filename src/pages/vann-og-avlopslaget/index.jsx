...
const Sewage = ({ data }) => {
  ...
  const [selectedYear, setSelectedYear] = useState(
    isCurrentYear ? currentYear : lastYear
  );
  ...
  return (
    <Layout menus={{ headerMenu, footerMenu }} seo={seo}>
      ...
      <ContentContainer
        size="medium"
        className={styles.annualMeetingsContainer}
      >
        <div className={styles.annualMeetingsSidebar}>
          <div
            className={styles.sidebarTitle}
            onClick={isMobile ? handleSidebar : null}
          >
            <h5>Velg årstall</h5>
            {isMobile && (
              <img
                src={
                  isOpen
                    ? "../media/icons/up.icon.svg"
                    : "../media/icons/down.icon.svg"
                }
            ...
            <ul className={styles.sidebarList}>
              {arsmoter
                ...
                .map((arsmote) => {
                  ...
                  return (
                    <li
                      className={`${isSelectedYear && styles.selected} ${
                        styles.listItem
                      }`}
                      onClick={handleYear}
                    >
                      <h5>{arsmote.title}</h5>
                      {isSelectedYear && (
                        <img src="../media/icons/right.icon.svg" alt="Valgt" />
                      )}
                    </li>
                  );
                })}
            ...
            if (title == selectedYear) {
              return (
                <>
                  ...
                  {arsmote.documents &&
                    arsmote.documents.map(
                      ({ title, date, file: { mediaItemUrl: file } }, ixd) => {
                        return (
                          <>
                            {file != null && (
                              <a
                                className={styles.annualMeeting}
                                href={file}
                                target="_blank"
                                key={ixd}
                              >
                                <img
                                  src="../media/icons/file.icon.svg"
                                  alt="PDF-file"
                                />
                                <h5>{title}</h5>
                                <h5>{date}</h5>
